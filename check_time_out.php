...
if (isset($_POST['reference_no']) && isset($_POST['time_out'])) {
    ...
    if (mysqli_stmt_execute($stmt)) {
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            ...
            $insertQuery = "INSERT INTO unsuccessful_appointment (fullname, sex, priority, phonenumber, scheduledate, appointment, purpose_of_visit, position_designation, agency_school_office, department, reference_no, time_in) 
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt = mysqli_prepare($conn, $insertQuery);
            mysqli_stmt_bind_param($stmt, "ssssssssssss", $row['fullname'], $row['sex'], $row['priority'], $row['phonenumber'], $row['scheduledate'], $row['appointment'], $row['purpose_of_visit'], $row['position_designation'], $row['agency_school_office'], $row['department'], $row['reference_no'], $row['time_in']);
